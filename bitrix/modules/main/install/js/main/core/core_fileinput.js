...
*/		if (values.length > 0)
	...
	checkUploadControl : function()
	{
		...
		if (!this.uploadParams['upload'])
		{
			...
			BX.removeClass(this.container, "adm-fileinput-drag-area");
			BX.addClass(this.container, "adm-fileinput-drag-area-error");
		}
		else if (this.uploadParams["maxCount"] <= 0 || this.uploadParams["maxCount"] > this.agent.getItems().length)
		{
			...
			BX.removeClass(this.container, "adm-fileinput-drag-area-error");
...
//			files : { created : [], ready : [] },
	...
	onUploadDone : function(it, data)
	{
		...
		if (item && BX(node))
		{
			var id = item.id + 'Value',
				...
				uploadId = (data && data["file"] && data["file"]["uploadId"] ? data["file"]["uploadId"] : ''),
				input2;
			...
			{
				BX.remove(BX.findChild(this.agent.form, {tagName : "INPUT", attr : {name : (input.name + '_del')}}, false));
			}
	...
	bindFile : function(item)
	{
		...
		if (BX(id + 'Edit') && !BX(id + 'Edit').hasAttribute("bx-bound"))
		{
			...
			BX.bind(BX(id + 'Edit'), "click", BX.delegate(function(e){
				BX.PreventDefault(e);
				this.editFile(item, BX.proxy_context);
			}, this));
		}
	...
	editFilePopup : null,
	editFile : function(item, node)
	{
		if (this.editFilePopup)
			this.editFilePopup.close();
		if (!this.editFilePopupApply)
		{
			this.editFilePopupApply = BX.delegate(function() {
				var id = BX('bx-edit-file-item-' + this.id).value,
					descr = BX('bx-edit-file-input-' + this.id).value,
					item = this.agent.getItem(id);
				this.editFilePopup.close();
				if (item)
				{
					if (BX(id + 'Description'))
						BX(id + 'Description').value = descr;
					item.item.description = descr;
				}
			}, this);
			this.editFilePopupDestroy = BX.delegate(function() {
				this.editFilePopup = null;
			}, this);
			this.editFilePopupBindCounter = 0;
			this.editFilePopupBind = BX.delegate(function(id) {
				if (BX('bx-edit-file-input-' + id))
				{
					this.editFilePopupBindCounter = 0;
					BX.focus(BX('bx-edit-file-input-' + id));
					BX.bind(BX('bx-edit-file-button-' + id), "click", this.editFilePopupApply);
				}
				else if (this.editFilePopupBindCounter < 10)
				{
					this.editFilePopupBindCounter++;
					BX.defer_proxy(this.editFilePopupBind)(id);
				}
			}, this);
		}
		this.editFilePopup = new BX.PopupWindow('bx-edit-file-' + this.id, node, {
			lightShadow : true,
			className : "adm-workarea adm-fileinput-popup",
			offsetTop: -54,
			offsetLeft: -10,
			autoHide: true,
			closeByEsc: true,
			...
			events : {
				...
				onPopupDestroy : this.editFilePopupDestroy
			},
			content : [
				'<input type="hidden" id="bx-edit-file-item-', this.id,'" value="', item.id,'" />',
				'<div class="adm-fileinput-popup-button-container"><input type="button" id="bx-edit-file-button-', this.id,'" value="', BX.message("JS_CORE_FILE_APPLY"),'"></div>',
				'<div class="adm-fileinput-popup-input-container"><input type="text" id="bx-edit-file-input-', this.id,'" placeholder="', BX.message("JS_CORE_FILE_DESCRIPTION"),'" value="', item.description,'"></div>'
			].join("")
		});
		this.editFilePopup.show();
		this.editFilePopupBind(this.id)
	},
...
var filePath = function(id, events)
{
	this.id = id;
	this.number = 0;
	this.templateNode = [
	...
	this.template = [
		'<div class="adm-fileinput-urls-container" id="#id#_container">',
			'<ol class="adm-fileinput-list adm-fileinput-urls" id="#id#_list">',
				...
				'</li>',
			'</ol>',
			'<a href="#" id="#id#_add_point" class="adm-fileinput-item-add">', BX.message("JS_CORE_FI_ADD_LINK"), '</a>',
...
FramePreset.prototype = {
	...
	edit : function(params)
	{
		...
		this.popup = new BX.PopupWindow('bx-preset-popup-' + node.id, node, {
			...
			autoHide: true,
			closeByEsc: true,
			zIndex : BX.PopupWindow.getOption("popupOverlayZindex") + 30,
...
FrameMaster.prototype = {
	...
	{
		if (this.activeItem != null && this.canvas.changed === true)
...
CanvasMaster.prototype = {
	...
	poster : function(node)
	{
		...
		this.posterPopup = new BX.PopupWindow('bx-poster-popup-' + node.id, node, {
			...
			autoHide: true,
			closeByEsc: true,
			zIndex : BX.PopupWindow.getOption("popupOverlayZindex") + 1,
			bindOptions: {position: "top"},
			overlay : false,
			events : {
				onPopupClose : function() { this.destroy() },
	...
	init : function()
	{
		if (this.preventer === null)
		{
			this.root = BX.create('DIV', {
				...
				style : {
					position : "absolute",
					zIndex : BX.PopupWindow.getOption("popupOverlayZindex") + 10,
	...
	else
	{
		this.root = BX.create('DIV', {
			style : {
				display : "none",
				position : "absolute",
				zIndex : BX.PopupWindow.getOption("popupOverlayZindex") + 20
			},
			attrs : {
				id : this.id + 'Root',
