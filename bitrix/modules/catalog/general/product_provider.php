...
//			"AVAILABLE_QUANTITY" => $arCatalogProduct["QUANTITY"],
	...
	public static function DeductProduct($arParams)
	{
		...
		$isOrderConverted = \Bitrix\Main\Config\Option::get('sale', 'order_converted', 'N');
		...
		if (isset($arParams["BASKET_ITEM"]) && $arParams["BASKET_ITEM"] instanceof \Bitrix\Sale\BasketItem)
		{
...
//						{
//							if (($totalAmount > $arProduct["QUANTITY_RESERVED"])
//								|| ($totalAmount > $arProduct["QUANTITY_RESERVED"] + $arProduct["QUANTITY"]))
//							{
//
//								$result->addError( new \Bitrix\Sale\ResultError(
//									                   Loc::getMessage("DDCT_UNKNOWN_ERROR", self::GetProductCatalogInfo($productId)),
...
//				$arRes["RESULT"] = true;
	...
	public static function tryUnshipmentProduct($productId)
	{
		...
		if (!$arProduct = $rsProducts->Fetch())
		{
			$result->addError( new \Bitrix\Sale\ResultError(Loc::getMessage(""), 'PROVIDER_UNSHIPMENT_PRODUCT') );
		}
