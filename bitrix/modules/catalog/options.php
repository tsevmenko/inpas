...
if ($USER->CanDoOperation('catalog_read') || !$bReadOnly)
{
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST" && !empty($_POST['Update']) && !$bReadOnly && check_bitrix_sessid())
	{
		...
		call_user_func_array((isset($_POST['product_form_simple_search']) && (string)$_POST['product_form_simple_search'] === 'Y' ? 'Un' : '').'RegisterModuleDependences', array(
			'search', 'BeforeIndex', 'catalog', '\Bitrix\Catalog\SearchHandlers', 'onBeforeIndex'
		));
